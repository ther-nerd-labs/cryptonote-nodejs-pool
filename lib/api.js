...
function collectStats(){
    ...
    async.parallel({
	    ...
	    service.getPoolStats(config.coin, config.api)
		.then(replies => {
		...
		service.getMaturedBlocks(config.coin)
		    .then(result => {
...
function broadcastWorkerStats(address, destinations) {
	service.getMinerStats(config.coin, address, config.api)
	    .then(replies => {
		    ...
		    service.getMinerChartData(config.coin, address, workersData)
			.then(replies => {
...
function handleMinerStats(urlParts, response){
	...
	service.getMinerExists(config.coin, address)
	    .then(result => {
                if (!result){
	    ...
	    .catch(error => {
		response.end(JSON.stringify({error: `Not found ${error}`}))
	    })
    }
    else{
	service.getMinerStats(config.coin, address, config.api)
	    .then(replies => {
			...
			.catch(error => {
		            log('error', logSystem, 'Error getUserChartsData %s', [error])
			})
                })
            })
	    .catch(error => {
...
function handleGetPayments(urlParts, response){
    service.getPayments(config.coin, config.api, urlParts.query.time, urlParts.query.address)
	.then(result => {
        ...
        .catch(error => {
            let reply = JSON.stringify({error: 'Query failed'})
            response.writeHead("200", {
                'Access-Control-Allow-Origin': '*',
                'Cache-Control': 'no-cache',
                'Content-Type': 'application/json',
...
function handleGetBlocks(urlParts, response){
    service.getMaturedBlocks(config.coin, urlParts.query.height, config.api.blocks)
	.then(result => {
            let reply = JSON.stringify(result)
            response.writeHead("200", {
                'Access-Control-Allow-Origin': '*',
...
function handleTopMiners(response) {
    service.getTopMiners(config.coin) 
        .then(redisData => {
            let minersData = []
            let keyParts = []
            let address = ''
            let data = ''
            for (let i in redisData[1]) {
                keyParts = redisData[0][i].split(':')
            ...
            minersData = minersData.slice(0,10)

            let reply = JSON.stringify(minersData)
            response.writeHead("200", {
                'Access-Control-Allow-Origin': '*',
                'Cache-Control': 'no-cache',
                'Content-Type': 'application/json',
...
function handleGetMinerPayoutLevel(urlParts, response){
    ...
    service.getMinerMinPayout(config.coin, address)
        .then(value => {
...
function handleSetMinerPayoutLevel(urlParts, response){
    ...
    minerSeenWithIPForAddress(address, ip, function (error, found) {
        ...
        service.setMinerMinPayout(config.coin, address, payoutLevel)
            .then(value => {
...
function handleGetMinerNotifications(urlParts, response){
    ...
    service.getMinerEmail(config.coin, address)
	.then(value => {
...
function handleSetMinerNotifications(urlParts, response){
    ...
    minerSeenWithIPForAddress(address, ip, function (error, found) {
	    ...
	    service.setMinerEmailAddress(config.coin, address, email)
		.then(value => {
	    ...
	    service.delMinerEmailAddress(config.coin, address)
		.then(value => {
...
function handleGetTelegramNotifications(urlParts, response){
    ...
    if (type == 'default') {
        service.getDefaultTelegramId(config.coin, chatId)
	    .then(value => {
    ...
    // Blocks notification
    if (type === 'blocks') {
	service.getTelegramBlocksByChatId(config.coin, chatId)
	    .then(value => {
	...
	service.getTelegramId(config.coin, address)
	    .then(value => {
...
function handleSetTelegramNotifications(urlParts, response){
	...
	service.setDefaultTelegramId(config.coin, chatId, address)
	    .then(value => {
	        response.end(JSON.stringify({status: 'done'}))
	    })
	    .catch(error => {
    ...
    if (type === 'blocks') {
        // Enable
        if (action === "enable") {
	    service.setTelegramBlocksByChatId(config.coin, chatId, 1)
		.then(value => {
	    ...
	    service.delTelegramBlocksByChatId(config.coin, chatId)
		.then(value => {
	...
	service.getMinerExists(config.coin, address)
		...
		if (!result) {
		    response.end(JSON.stringify({status: 'Miner not found in database'}))
                ...
                // Enable
                if (action === "enable") {
		    service.setTelegramId(config.coin, address, chatId)
			.then(value => {
		    ...
		    service.delTelegramId(config.coin, address)
			.then(value => {
...
function handleAdminStats(response){
    ...
    service.getStatsAdmin(config.coin)
	.then(workerDataAndBlocks => {
            let stats = {
                totalOwed: 0,
                totalPaid: 0,
                totalRevenue: 0,
	    ...
	    let workerData = workerDataAndBlocks[0]
	    let blocks = workerDataAndBlocks[1]
...
function handleAdminUsers(response){
    ...
    service.getUserStatsAdmin(config.coin)
	.then(replies => {
            let workersData = {}
            let keyParts = []
            let address = ''
            let data = []
	    let workerKeys = replies[0]
	    let redisData = replies[1]
	...
	.catch(error => {
	    response.end(JSON.stringify({error: 'Error collection users stats'}))
	    return
	})
}
...
function handleAdminPorts(response){
    service.getUserPortsAdmin(config.coin)
	.then(keysAndPorts => {
	    let portsKeys = keysAndPorts[0]
	    let redisData = keysAndPorts[1]
            let portsData = {}
            let port = ''
            let data = []
            for (let i in redisData) {
	...
	.catch(error => {
	    response.end(JSON.stringify({errors: 'Error collection Ports stats'}))
	    return
	})
}
...
function startRpcMonitoring(rpc, module, method, interval) {
    setInterval(function() {
        rpc(method, {}, function(error, response) {
            ...
            if(error) {
                stats.lastFail = stats.lastCheck
                stats.lastFailResponse = stats.lastResponse
            }
	    service.setMonitoringStatus(config.coin, module, stats)
		.then(value => {})
		.catch(error => {})
        })
    }, interval * 1000)
...
function getMonitoringData(callback) {
    ...
    let redisCommands = []
    service.getMonitoringStatus(config.coin, modules)
	.then(results => {
	    let stats = {}
...
function minerSeenWithIPForAddress(address, ip, callback) {
    ...
    service.findMinerIPByAddress(config.coin, address, ip)
	.then(result => {
	    callback(null, result > 0 ? true : false)
	})
	.catch(error => {
