...
*/    
        data['uri'] = `${port === 443 ? https : http}://${host}:${port}${path}`
        data['method'] = data ? 'POST' : 'GET'
        data['header'] = { 
                        'Content-Length': data.length,
                        'Content-Type': 'application/json',
                        'Accept': 'application/json'
 ...
 console.log(data)
        request(data)
            .then(function(response){
                    callback(null, response ? JSON.parse(response) : {})
                })
    ...
    return {
        ...
        rpcWallet: function(method, params, callback){
            rpc(walletConfig.host, walletConfig.port, method, params, callback,
                walletConfig.username, walletConfig.password);
        },
