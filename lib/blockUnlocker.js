        ...
        function(blocks, callback){
            ...
            blocks.forEach(function(block){
                ...
                if (block.workerScores) {
                    ...
                    if(config.poolServer.payoutScheme && config.poolServer.payoutScheme == "window"){

                        log('info', logSystem, 'scheme is window');

                        Object.keys(block.workerScores).forEach(function (worker) {
                            
                            /* add up all worker scores to make total */
                            log('info', logSystem, 'adding ' + block.workerScores[worker] + ' to total score');
                            totalScore += block.workerScores[worker];

                        });

                        log('info', logSystem, 'total sum score is ' + totalScore);
                        log('info', logSystem, 'total block score is ' + parseFloat(block.score));

                    }else{

                        log('info', logSystem, 'scheme is not window');
                        totalScore = parseFloat(block.score);
