...
function sendToMinerTelegram(miner, id, variables) { 
    ...
    if (config.telegram && config.telegram.enabled) {
        ...
        let chatId = service.getTelegramId(config.coin, miner)
        if (!chatId) return;
        telegram.sendMessage(chatId, message);
    }
}
...
function sendBlockTelegram(id, variables) {
    ...
    if (config.telegram && config.telegram.enabled) {
        ...
        let data = service.getTelegramBlockData(config.coin)
        if (!data) return ;
...
function sendToAllEmails(id, variables) {
	...
	let data = service.getAllEmails(config.coin)
        if (!data) return ;
...
function sendToMinerEmail(miner, id, variables) {
	...
	let email = service.getMinerEmail(config.coin, miner)
        if (!email) return ;
        emailSystem.sendEmail(email, subject, content);
    }
}
