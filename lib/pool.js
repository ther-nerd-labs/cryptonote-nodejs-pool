...
const PAYOUT_SCHEME_SLUSH = "slush";
const PAYOUT_SCHEME_WINDOW = "window";
    ...
    switch(method){
        case 'login':
            ...
            if (!utils.validateMinerAddress(address)) {
                ...
                sendReply('Invalid address used for login');
                return;
            }else if(addr == null || (addr.length > 1 && (addr[1] == null || addr[1].match(/[^A-Za-z0-9]/g) != null))){
                log('warn', logSystem, 'Invalid payment id used for login %s', addr[1]);
                sendReply('Invalid payment id used for login');
                return;
            }
...
function recordShareData(miner, job, shareDiff, blockCandidate, hashHex, shareType, blockTemplate){
    ...
    let updateScore = generateScoreUpdate(miner, job, shareDiff, blockCandidate, hashHex, shareType, blockTemplate);
    ...
    if (blockCandidate){
        ...
        if(config.poolServer.payoutScheme == PAYOUT_SCHEME_WINDOW){

            resetMinerStartTimes();
    ...
    redisClient.multi(redisCommands).exec(function(err, replies){
        ...
        if (config.poolServer.payoutScheme == PAYOUT_SCHEME_SLUSH) {
...
/**
 * calculate score based on current configured payment system
 */
function generateScoreUpdate(miner, job, shareDiff, blockCandidate, hashHex, shareType, blockTemplate){

    var scheme = config.poolServer.payoutScheme ? config.poolServer.payoutScheme : '';

    /* compatibility with existing setups, config must not have slush mining enabled for other payments schemes to work */
    scheme = slushMiningEnabled ? PAYOUT_SCHEME_SLUSH : scheme;

    log('info', logSystem, 'using payout scheme ' + scheme);

    switch(scheme){

        case PAYOUT_SCHEME_WINDOW:

            /**
             * shares weighted by time mining since login (miners window), login time is set to current time at login, and when block found.
             * disconnecting will cause miners to lose accumalated share wight, loyal miners will continue to accumalate share weight so hoppers
             * perecentage of reward will deminish. if hoppers come back later in the round, their weights are reset so they have to start again. 
             * 
             * converting milliseconds into hours to avoid massive numbers.
             */

            var weight = ((new Date().getTime() - miner.startTime) / 3600000);

            log('info', logSystem, 'start time was ' + miner.startTime);
            log('info', logSystem, 'share weight is ' + weight);

            job.score = weight * job.difficulty;

            log('info', logSystem, 'share score is ' + job.score);

            break;

        case PAYOUT_SCHEME_SLUSH:
        
            log('info', logSystem, 'using payout scheme slush');


            /* original slush mining setup, returns a different redis command so return here */
            return generateSlushMiningEvalScript(miner, job, shareDiff, blockCandidate, hashHex, shareType, blockTemplate);
        
        case '':
        default:

            log('info', logSystem, 'using payout scheme default');


            /* original proportional setup */
            job.score = job.difficulty;


    }

    return ['hincrbyfloat', config.coin + ':scores:roundCurrent', miner.login, job.score]

}

/**
 * generate eval script for slush mining
 */
function generateSlushMiningEvalScript(miner, job, shareDiff, blockCandidate, hashHex, shareType, blockTemplate){
...
/**
 * resets start times for all miners, may be usefull for some payment systems
 */
function resetMinerStartTimes(){

    var date = new Date();
    var count = 0;

    for(miner in connectedMiners){

        miner.startTime = date.getTime();
        count++;

    }

    log('info', logSystem, 'reset ' + count + " miners start times");
