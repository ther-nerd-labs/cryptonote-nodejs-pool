...
function newConnectedWorker(miner){
    ...
    if (service.recordWorker(config.coin, miner) === 1) {
...
function removeConnectedWorker(miner, reason){
    ...
    let connectedWorkers = service.removeWorker(config.coin, miner)
    if (reason === 'banned') {
...
function recordShareData(miner, job, shareDiff, blockCandidate, hashHex, shareType, blockTemplate){
    ...
    let replies = service.recordShareData(config.coin, job, slushMiningEnabled, dateNow, miner, cleanupInterval, blockCandidate)
    if (!replies) return
	...
	service.recordBlockCandidate(config.coin, hashHex, dateNowSeconds, blockTemplate.difficulty, totalShares, totalScore)
